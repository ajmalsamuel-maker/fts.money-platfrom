...
Deno.serve(async (req) => {
    ...
    try {
        ...
        console.log(`Recreating merchants table in schema: ${schemaName}`);

        // Drop existing table
        await client.queryObject(`DROP TABLE IF EXISTS "${schemaName}".merchants CASCADE`);
        console.log('✅ Dropped old merchants table');

        // Create merchants table with all required columns
        await client.queryObject(
            `CREATE TABLE "${schemaName}".merchants (
        ...
        // Create indexes
        await client.queryObject(`CREATE INDEX idx_merchants_psp_code ON "${schemaName}".merchants(psp_code)`);
        await client.queryObject(`CREATE INDEX idx_merchants_merchant_code ON "${schemaName}".merchants(merchant_code)`);
        await client.queryObject(`CREATE INDEX idx_merchants_status ON "${schemaName}".merchants(status)`);

        console.log('✅ Merchants table recreated successfully');

        return Response.json({
            success: true,
            message: `Merchants table recreated in ${schemaName}`
        });

    } catch (error) {
        console.error('Error recreating merchants table:', error);
        return Response.json({
            success: false,
            error: error.message
        }, { status: 500 });
