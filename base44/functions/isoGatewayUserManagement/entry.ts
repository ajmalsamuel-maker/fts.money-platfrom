...
Deno.serve(async (req) => {
    try {
        ...
        if (action === 'invite') {
            const existing = await base44.asServiceRole.entities.ISOGatewayUser.filter({ customer_id, email });
            if (existing.length > 0) return Response.json({ success: false, error: 'User already exists' });

            const password_hash = await hashPassword(password);
            await base44.asServiceRole.entities.ISOGatewayUser.create({ customer_id, email, full_name, role, password_hash, status: 'active' });
