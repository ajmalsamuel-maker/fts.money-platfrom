...
export default function ISOGatewayUserManagement() {
    ...
    const [inviteForm, setInviteForm] = useState({ customer_id: '', email: '', full_name: '', role: ISO_ROLES.VIEWER, password: '' });
    const [editForm, setEditForm] = useState({ full_name: '', role: '' });
    const [error, setError] = useState('');

    const { data: customers = [] } = useQuery({
        queryKey: ['iso-customers'],
        queryFn: async () => await base44.asServiceRole.entities.ISOGatewayCustomer.list()
    });

    const { data: users = [] } = useQuery({
        queryKey: ['iso-users'],
        queryFn: async () => await base44.asServiceRole.entities.ISOGatewayUser.list()
    });

    const inviteMutation = useMutation({
        mutationFn: async (userData) => {
            const response = await base44.functions.invoke('isoGatewayUserManagement', { action: 'invite', ...userData });
    ...
    const updateMutation = useMutation({
        mutationFn: async ({ id, data }) => await base44.asServiceRole.entities.ISOGatewayUser.update(id, data),
        onSuccess: () => {
            queryClient.invalidateQueries(['iso-users']);
            setEditOpen(false);
            setEditingUser(null);
    ...
    const deleteMutation = useMutation({
        mutationFn: async (id) => await base44.asServiceRole.entities.ISOGatewayUser.delete(id),
        onSuccess: () => {
            queryClient.invalidateQueries(['iso-users']);
            setDeleteUser(null);
        }
        ...
        <div className="flex h-screen bg-slate-50">
            ...
            <div className="flex-1 overflow-auto">
                <header className="h-16 bg-white border-b flex items-center justify-between px-6">
                    <div>
                        <h2 className="text-lg font-semibold">ISO Gateway User Management</h2>
                        <p className="text-xs text-slate-600">Manage users across ISO Gateway customers</p>
                    </div>
                    <Dialog open={inviteOpen} onOpenChange={setInviteOpen}>
                        <DialogTrigger asChild>
                            <Button className="gap-2 bg-indigo-600 hover:bg-indigo-700"><UserPlus className="h-4 w-4" />Invite User</Button>
                        </DialogTrigger>
                        <DialogContent>
                            <DialogHeader><DialogTitle>Invite ISO Gateway User</DialogTitle></DialogHeader>
                            <div className="space-y-4 mt-4">
                                ...
                                <div><Label>Role</Label><Select value={inviteForm.role} onValueChange={(v) => setInviteForm({...inviteForm, role: v})}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{Object.values(ISO_ROLES).map(role => <SelectItem key={role} value={role}>{getISORoleLabel(role)}</SelectItem>)}</SelectContent></Select></div>
                ...
                <div className="p-6">
                    <Card>
                        <CardHeader><CardTitle className="flex items-center gap-2"><Code className="h-5 w-5" />ISO Gateway Users ({users.length})</CardTitle></CardHeader>
                        <CardContent>
                                    ...
                                    return (
                                        <div key={user.id} className="flex items-center justify-between p-4 border rounded-lg">
                                            ...
                                            <div className="flex items-center gap-3">
                                                <Badge className={user.role === ISO_ROLES.OWNER ? 'bg-red-100 text-red-700' : 'bg-indigo-100 text-indigo-700'}>{getISORoleLabel(user.role)}</Badge>
            ...
            <Dialog open={editOpen} onOpenChange={setEditOpen}>
                <DialogContent>
                    ...
                    <div className="space-y-4 mt-4">
                        ...
                        <div><Label>Role</Label><Select value={editForm.role} onValueChange={(v) => setEditForm({...editForm, role: v})}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{Object.values(ISO_ROLES).map(role => <SelectItem key={role} value={role}>{getISORoleLabel(role)}</SelectItem>)}</SelectContent></Select></div>
