...
import { Users, UserPlus, Trash2, Pencil, Rocket } from 'lucide-react';
...
export default function RWAProviderUserManagement() {
    ...
    const inviteMutation = useMutation({
        mutationFn: async (userData) => {
            const response = await base44.functions.invoke('rwaProviderUserManagement', { action: 'invite', ...userData });
    ...
    return (
        <div className="flex h-screen bg-slate-50">
            ...
            <div className="flex-1 overflow-auto">
                ...
                <div className="p-6">
                    <Card>
                        <CardHeader><CardTitle className="flex items-center gap-2"><Rocket className="h-5 w-5" />RWA Platform Users ({users.length})</CardTitle></CardHeader>
                        <CardContent>
                                    ...
                                    return (
                                        <div key={user.id} className="flex items-center justify-between p-4 border rounded-lg">
                                            <div>
                                                ...
                                                <p className="text-xs text-slate-500">{provider?.company_name}</p>
                                            </div>
                                            <div className="flex items-center gap-3">
                                                <Badge className={user.role === RWA_ROLES.OWNER ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700'}>{getRWARoleLabel(user.role)}</Badge>
